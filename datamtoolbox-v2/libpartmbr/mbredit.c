...
static int do_editloc(int entry,uint32_t start,uint32_t num,int type) {
	...
	if (_polyfill_lseek(diskimage_fd,0,SEEK_SET) != 0 || write(diskimage_fd,diskimage_sector,LIBPARTMBR_SECTOR_SIZE) != LIBPARTMBR_SECTOR_SIZE) {
...
static int do_ext_list(uint32_t first_lba) {
	...
	while (1) {
		...
		if (_polyfill_lseek(diskimage_fd,seekofs,SEEK_SET) != seekofs || read(diskimage_fd,sector,LIBPARTMBR_SECTOR_SIZE) != LIBPARTMBR_SECTOR_SIZE)
			break;
